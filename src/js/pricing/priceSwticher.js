export const usePriceSwitcher = () => {
  const switcher = document.querySelector('[data-price="swticher"]');
  const starterPrice = document.querySelector('[data-price="starter"]');
  const popularPrice = document.querySelector('[data-price="popular"]');
  const entriesPrice = document.querySelector('[data-price="enterprise"]');
  const starterPeriod = document.querySelector('[data-period="starter"]');
  const popularPeriod = document.querySelector('[data-period="popular"]');
  const entriesPeriod = document.querySelector('[data-period="enterprise"]');

  const priceList = {
    starter: {
      default: 120,
      witchSale: 100*12,
			mounthPeriod: '/mo',
			yearPeriod: '/yr'
    },
    popular: {
			default: 1680,
      witchSale: 1400*12,
			mounthPeriod: '/mo',
			yearPeriod: '/yr'
    },
    enterprise: {
			default: 2520,
      wtichSale: 2100*12,
			mounthPeriod: '/mo',
			yearPeriod: '/yr'
    },
  };

  const setPricesWitchSale = () => {
    starterPrice.textContent = priceList.starter.witchSale;
    popularPrice.textContent = priceList.popular.witchSale;
    entriesPrice.textContent = priceList.enterprise.wtichSale;
    starterPeriod.textContent = priceList.starter.yearPeriod;
    popularPeriod.textContent = priceList.popular.yearPeriod;
    entriesPeriod.textContent = priceList.enterprise.yearPeriod;
  };

  const setDefaultPrices = () => {
    starterPrice.textContent = priceList.starter.default;
    popularPrice.textContent = priceList.popular.default;
    entriesPrice.textContent = priceList.enterprise.default;
		starterPeriod.textContent = priceList.starter.mounthPeriod;
    popularPeriod.textContent = priceList.popular.mounthPeriod;
    entriesPeriod.textContent = priceList.enterprise.mounthPeriod;
  };

  switcher.checked = true;
  setPricesWitchSale();

  switcher.addEventListener('click', () => {
    if (switcher.checked) {
      setPricesWitchSale();
    } else {
      setDefaultPrices();
    }
  });
};
